...
if ! netstat -n | grep -q 5900; then
  ...
  # echo "$(date) Exiting early; no screen-sharing session"
  exit 0
fi

# Exit early if host resolution has already been set
if [ -f "${HOME}/.res.tmp" ]; then
  # echo "$(date) Exiting early; resolution has been set"
  exit 0
fi
...
echo "$(date) Getting hostname of the client at ${client_ip}"
client_hostname=$(nslookup ${client_ip} | sed -En "s/^.*name = (.*).$/\1/p")

# Get the resolution of the client (e.g. "1280 800")
echo "$(date) Getting screen resolution of the client at ${client_hostname}"
...
echo "$(date) Obtained client resolution: ${client_width}x${client_height}"
...
echo "$(date) Caching host resolution"
touch "${HOME}/.res.tmp"